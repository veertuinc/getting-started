...
BLOCK
  ...
  docker exec -i anka.gitlab bash -c "gitlab-rails runner \"token = User.find_by_username('root').personal_access_tokens.create(scopes: [:read_user, :read_repository, :api], name: 'Automation token', expires_at: Time.now + 10.month); token.set_token('${GITLAB_ACCESS_TOKEN}'); token.save!\""
  
  ## Create example project
  echo "]] Importing example project"
  sleep 20
  curl -s --request PUT -H "PRIVATE-TOKEN: $GITLAB_ACCESS_TOKEN" "${URL_PROTOCOL}$GITLAB_DOCKER_CONTAINER_NAME:$GITLAB_PORT/api/v4/application/settings?import_sources%5B%5D=git,github"
  sleep 30
