...
if [[ $1 != "--uninstall" ]]; then
  ...
  eval docker run --name $GITLAB_RUNNER_SHARED_RUNNER_NAME $VOLUMES -ti -d veertu/anka-gitlab-runner-amd64:v$GITLAB_ANKA_RUNNER_VERSION \
  ...
  eval docker run --name $GITLAB_RUNNER_PROJECT_RUNNER_NAME $VOLUMES -ti -d veertu/anka-gitlab-runner-amd64:v$GITLAB_ANKA_RUNNER_VERSION \
  --url "http://${DOCKER_HOST_ADDRESS}:$GITLAB_PORT" \
