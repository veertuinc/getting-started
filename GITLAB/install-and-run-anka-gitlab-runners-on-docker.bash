...
if [[ $1 != "--uninstall" ]]; then
  ...
  # GitLab Runner
  DOCKER_RUN_EXTRAS="--rm -ti -d veertu/anka-gitlab-runner-amd64:v${GITLAB_ANKA_RUNNER_VERSION} ${VOLUMES}"
  ...
  eval docker run ${DOCKER_RUN_EXTRAS} --name $GITLAB_RUNNER_SHARED_RUNNER_NAME \
  ...
  eval docker run ${DOCKER_RUN_EXTRAS} --name $GITLAB_RUNNER_PROJECT_RUNNER_NAME \
  --url "http://${DOCKER_HOST_ADDRESS}:$GITLAB_PORT" \
