...
if [[ $1 != "--uninstall" ]]; then
  ...
  # GitLab Runner
  [[ "$(uname)" == "Linux" ]] && DOCKER_RUN_EXTRAS="${DOCKER_RUN_EXTRAS} --add-host=\"host.docker.internal:host-gateway\""
  ...
  eval docker run ${DOCKER_RUN_EXTRAS} --name $GITLAB_RUNNER_SHARED_RUNNER_NAME $VOLUMES -ti -d veertu/anka-gitlab-runner-amd64:v$GITLAB_ANKA_RUNNER_VERSION \
  ...
  eval docker run ${DOCKER_RUN_EXTRAS} --name $GITLAB_RUNNER_PROJECT_RUNNER_NAME $VOLUMES -ti -d veertu/anka-gitlab-runner-amd64:v$GITLAB_ANKA_RUNNER_VERSION \
  --url "http://${DOCKER_HOST_ADDRESS}:$GITLAB_PORT" \
