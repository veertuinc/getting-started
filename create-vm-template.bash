...
if [[ "${FLAGS}" != "--no-anka-create" ]]; then
  ...
  RETRIES=4
  NEXT_WAIT_TIME=0
  until [ ${NEXT_WAIT_TIME} -eq ${RETRIES} ] || timeout 14400 bash -c "time ${SUDO} ANKA_CREATE_SUSPEND=0 anka ${ANKA_DEBUG} create --disk-size 100G --app \"$INSTALLER_LOCATION\" $TEMPLATE_NAME"; do
