...
FOUND_MIST_MACOS_VERSION="$(mist list ${MIST_OPTIONS} --latest -o json -q | jq -r '.[].version')"
...
if [[ ! -d "${INSTALL_MACOS_DIR}/${PREFIX_FOR_INSTALLERS}${FOUND_MIST_MACOS_VERSION}${EXTENSION}" ]]; then
  if [[ -z "${FOUND_MIST_MACOS_VERSION}" ]]; then
    ...
    sudo mist download ${MIST_OPTIONS} ${MIST_APPLICATION} --application-name "${PREFIX_FOR_INSTALLERS}%VERSION%${EXTENSION}" --output-directory "${INSTALL_MACOS_DIR}" > "${LOG_LOC}" # jenkins log becomes unreasonably large if we show all of the output while downloading
