...
openssl req -new -nodes -x509 -sha256 -days 365 -keyout anka-ca-key.pem -out anka-ca-crt.pem -subj "/C=$COUNTRY/ST=$STATE/L=$LOCATION/O=$ORGANIZATION/OU=$ORG_UNIT/CN=$CA_CN"
...
openssl req -new -nodes -sha256 -key anka-controller-key.pem -out anka-controller-csr.pem -subj "/C=$COUNTRY/ST=$STATE/L=$LOCATION/O=$ORGANIZATION/OU=$ORG_UNIT/CN=$CONTROLLER_CN" -reqexts SAN -extensions SAN -config <(cat /etc/ssl/openssl.cnf <(printf "[SAN]\nextendedKeyUsage = serverAuth\nsubjectAltName=IP:$CONTROLLER_SERVER_IP,DNS:anka.controller,DNS:anka.registry,DNS:host.docker.internal"))
openssl x509 -req -days 365 -sha256 -in anka-controller-csr.pem -CA anka-ca-crt.pem -CAkey anka-ca-key.pem -CAcreateserial -out anka-controller-crt.pem -extfile <(echo subjectAltName = IP:$CONTROLLER_SERVER_IP,DNS:anka.controller,DNS:anka.registry,DNS:host.docker.internal)
...
for NODE_NAME in "${NODE_NAMES[@]}"; do
  ...
  openssl req -new -sha256 -key anka-node-$NODE_NAME-key.pem -out anka-node-$NODE_NAME-csr.pem -subj "/C=$COUNTRY/ST=$STATE/L=$LOCATION/O=$ORGANIZATION/OU=$ORG_UNIT/CN=$NODE_NAME"
  ...
  echo "sudo ankacluster join https://127.0.0.1:8080 --cert $CERT_DIRECTORY/anka-node-$NODE_NAME-crt.pem --cert-key $CERT_DIRECTORY/anka-node-$NODE_NAME-key.pem --cacert $CERT_DIRECTORY/anka-ca-crt.pem"
  echo "curl -v https://127.0.0.1:8080/api/v1/status --cert $CERT_DIRECTORY/anka-node-$NODE_NAME-crt.pem --key $CERT_DIRECTORY/anka-node-$NODE_NAME-key.pem"
done
IFS=$OLDIFS
echo "[Creating Jenkins Cert]"
NAME="jenkins"
openssl genrsa -out anka-$NAME-key.pem 4096
openssl req -new -sha256 -key anka-$NAME-key.pem -out anka-$NAME-csr.pem -subj "/C=$COUNTRY/ST=$STATE/L=$LOCATION/O=$ORGANIZATION/OU=$ORG_UNIT/CN=$(tr '[:lower:]' '[:upper:]' <<< ${NAME:0:1})${NAME:1})"
...
echo "[Creating Gitlab Cert]"
NAME="gitlab"
openssl genrsa -out anka-$NAME-key.pem 4096
openssl req -new -sha256 -key anka-$NAME-key.pem -out anka-$NAME-csr.pem -subj "/C=$COUNTRY/ST=$STATE/L=$LOCATION/O=$ORGANIZATION/OU=$ORG_UNIT/CN=$(tr '[:lower:]' '[:upper:]' <<< ${NAME:0:1})${NAME:1})"
openssl x509 -req -days 365 -sha256 -in anka-$NAME-csr.pem -CA anka-ca-crt.pem -CAkey anka-ca-key.pem -CAcreateserial -out anka-$NAME-crt.pem
echo "[Creating Teamcity Cert]"
NAME="teamcity"
openssl genrsa -out anka-$NAME-key.pem 4096
openssl req -new -sha256 -key anka-$NAME-key.pem -out anka-$NAME-csr.pem -subj "/C=$COUNTRY/ST=$STATE/L=$LOCATION/O=$ORGANIZATION/OU=$ORG_UNIT/CN=$(tr '[:lower:]' '[:upper:]' <<< ${NAME:0:1})${NAME:1})"
openssl x509 -req -days 365 -sha256 -in anka-$NAME-csr.pem -CA anka-ca-crt.pem -CAkey anka-ca-key.pem -CAcreateserial -out anka-$NAME-crt.pem
